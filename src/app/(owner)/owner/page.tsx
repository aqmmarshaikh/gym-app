...
export default function OwnerLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!username.trim() || !password.trim()) {
      setError("Please fill out all executive credentials.");
      return;
    }
    ...
    setTimeout(async () => {
      const res = await dbService.validateOwnerLogin(username, password);
      setLoading(false);
    ...
    <div className="flex-1 flex flex-col justify-center px-6 py-12 bg-black min-h-screen text-white">
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="relative w-20 h-20 mb-3">
          <Image
            ...
            alt="Logo"
            fill
            className="object-contain filter drop-shadow-[0_0_15px_rgba(212,175,55,0.25)]"
          />
        </div>
        <h2 className="text-xl font-bold uppercase tracking-widest text-white">Owner Console</h2>
        <p className="text-xs text-text-secondary mt-1">Enterprise credentials required</p>
      </div>
      ...
      >
        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-semibold text-text-secondary uppercase mb-2">Executive Username</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-text-secondary uppercase mb-2">Security Key</label>
            <div className="relative">
